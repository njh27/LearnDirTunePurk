...
def get_all_mean_data(f_regex, directory, time_window, base_block,
    ...
    all_post_tuning_x_vel = {block: {curr_set: [] for curr_set in four_dir_trial_sets} for block in post_blocks}
    all_post_tuning_y_vel = {block: {curr_set: [] for curr_set in four_dir_trial_sets} for block in post_blocks}
        ...
        for block in post_blocks:
            ...
            for curr_set in four_dir_trial_sets:
                ...
                x, y = ab.subtract_baseline_tuning(ldp_sess, base_block, curr_set,
                                                   "eye velocity", x, y)
                # Append the data
                all_post_tuning_x_vel[block][curr_set].append(x)
                all_post_tuning_y_vel[block][curr_set].append(y)
    ...
    for block in post_blocks:
        for curr_set in four_dir_trial_sets:
            all_post_tuning_x_vel[block][curr_set] = np.vstack(all_post_tuning_x_vel[block][curr_set])
            all_post_tuning_y_vel[block][curr_set] = np.vstack(all_post_tuning_y_vel[block][curr_set])
...
def plot_comb_post_tuning_position_xy(data):
    ...
    for block in post_blocks:
        for curr_set in four_dir_trial_sets:
            # plt.plot(np.nanmean(data['post_tuning_x_pos'][block][curr_set], axis=0),
...
def plot_comb_tuning_probe_velocity_traces(data, time_window):
    ...
    trial_set_base_axis = {'learning': 1,
                       'anti_learning': 1,
                       'pursuit': 0,
                       'anti_pursuit': 0,
                       'instruction': 1
...
def plot_comb_post_tuning_velocity_traces(data, time_window):
    ...
    for block in post_blocks:
        ...
        means_y[block] = {}
        for curr_set in four_dir_trial_sets:
            means_x[block][curr_set] = np.nanmean(data['post_tuning_x_vel'][block][curr_set], axis=0)
            means_y[block][curr_set] = np.nanmean(data['post_tuning_y_vel'][block][curr_set], axis=0)
    trial_set_base_axis = {'learning': 0,
                       'anti_learning': 0,
                       'pursuit': 1,
                       'anti_pursuit': 1,
    ...
    for block in post_blocks:
        for curr_set in four_dir_trial_sets:
            ...
            if plot_axis == 0:
                post_pursuit_ax.plot(time, means_x[block][curr_set], color=p_col[block], linestyle=p_style[curr_set], label=line_label)
            elif plot_axis == 1:
                post_learn_ax.plot(time, means_y[block][curr_set], color=p_col[block], linestyle=p_style[curr_set], label=line_label)
            else:
                raise ValueError("Unrecognized trial set {0}.".format(curr_set))
